...
class array
{
private:              //data stucture parameter 
	int* A = {2,5,6,7,3};
	int size;
	int length;
public:
	...
	void dispaly()      //arr is the object to call
	{
		int i;
		printf("Elements are\n");
		for (i = 0; i < length; i++)
	...
	void append(int x)        //call by arr , the entering value
	{
		if (length < size)      //check is there any free space here 
			A[length++] = x;
	}

	//3. value insert at any place
	void insert(int index, int x)     //index is the array location and x is the value
	{
		int i;
		if (index >= 0 && index <= length)
		{
			for (i = length; i > index; i--)
				A[i] = A[i - 1];     //first the move place then the value place
			A[index] = x;
			length++;
		}
	}
	...
	int Delete(int index)
	{
		...
		if (index >= 0 && index < length)
		{
			...
			for (i = index; i < length - 1; i++)
				A[i] = A[i + 1];  //shift the value in empty space
			length--;
	...
	{
		int i;
		for (i = 0; i <=length - 1; i++)//  -> to accessing the member
		{
			if (key == A[i]) //to match all the element with key value
			{
				swap(&A[i], &A[i - 1]); //A[0] instade of A[i-1] will shift it first
				return i;
			}
		}
		return -1;
...
int main()
{
	array arr1;  //array inputs
//20 is size and 5 is data length

	//array* arr2 = { {3,4,5,1,6},10,5};
	//array* arr3;// this array for merge
